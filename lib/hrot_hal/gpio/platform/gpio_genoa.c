...
#define LOG_MODULE_NAME gpio_genoa
...
static void GenoaPchHold(void)
{
	pch_rst_enable_ctrl(true);
}

static void GenoaPchRelease(void)
{
	pch_rst_enable_ctrl(false);
}

static const struct platform_gpio_ctrl_ops genoa_gpio_ops = {
	.pch_hold = GenoaPchHold,
	.pch_release = GenoaPchRelease,
};

const struct platform_gpio_ctrl_ops *get_platform_gpio_ctrl_ops(void)
{
	return &genoa_gpio_ops;
}
