...
/* Keep RequestFirmwareData responses within the PFR MCTP reassembly buffer.
 * The generic protocol code adds its completion code and PLDM headers.
 */
...
struct pldm_fw_update_cfg {
	uint32_t image_size;
	/* RequestUpdate encodes this as uint32_t.  Keep the full width until it
	 * has been clamped to MAX_FWUPDATE_RSP_BUF_SIZE.
	 */
