...
static struct k_thread activate_reboot_thread;
K_KERNEL_STACK_MEMBER(activate_reboot_stack, ACTIVATE_REBOOT_STACK_SIZE);
...
uint8_t pldm_fw_update(void *fw_update_param, const int flash_position)
{
	...
	p->next_len = p->fw_update_cfg.max_buff_size;

	if (p->next_ofs < p->fw_update_cfg.image_size) {
		if (p->next_ofs + p->next_len > p->fw_update_cfg.image_size)
			p->next_len = p->fw_update_cfg.image_size - p->next_ofs;
...
void req_fw_update_handler(void *mctp_p, void *ext_params, void *arg)
{
	...
	update_param.fw_update_cfg.image_size = fw_update_cfg.image_size;
	update_param.fw_update_cfg.max_buff_size = fw_update_cfg.max_buff_size;
	...
	struct pldm_request_firmware_data_req req = { .offset = 0,
						      .length = update_param.fw_update_cfg.max_buff_size };
	...
	do {
		...
		if (req.offset + req.length >
		    update_param.fw_update_cfg.image_size + MIN_FW_UPDATE_BASELINE_TRANS_SIZE) {
			LOG_WRN("Request length over UA padding limit count 0x%x",
				MIN_FW_UPDATE_BASELINE_TRANS_SIZE);
			req.length = update_param.fw_update_cfg.image_size - req.offset;
		...
		if (req.length < MIN_FW_UPDATE_BASELINE_TRANS_SIZE) {
			...
			req.length = MIN_FW_UPDATE_BASELINE_TRANS_SIZE;
		} else if (req.length > update_param.fw_update_cfg.max_buff_size) {
			LOG_WRN("Request length larger than maximum size, modify it from 0x%x to 0x%x",
				req.length, update_param.fw_update_cfg.max_buff_size);
			req.length = update_param.fw_update_cfg.max_buff_size;
			expect_len = req.length;
		} else {
			expect_len = req.length;
		...
		uint16_t read_len =
			pldm_fw_update_read(mctp_p, PLDM_FW_UPDATE_CMD_CODE_REQUEST_FIRMWARE_DATA,
					    ...
					    /* ARRAY_SIZE(resp_buf) */ req.length + 1, ext_params);
		...
		if (!update_param.next_len) {
			LOG_INF("All data has been transferred for component %d", cur_update_comp_id);
			break;
		}
			      ...
			      uint8_t *resp, uint16_t *resp_len, void *ext_params)
{
	...
	*resp_len = 1;
	...
	if (len != (sizeof(struct pldm_request_update_req) + req_p->comp_image_set_ver_str_len)) {
		LOG_ERR("update_request len = %d expected = %d + %d",
				len,
				    ...
				    void *ext_params)
{
	...
	*resp_len = 1;
	...
	switch (req_p->comp_classification) {
...
static void activate_reboot_handler(void *arg1, void *arg2, void *arg3)
{
	ARG_UNUSED(arg1);
	ARG_UNUSED(arg2);
	ARG_UNUSED(arg3);

	LOG_INF("No reboot observed %d seconds after ActivateFirmware, forcing chip reset",
		ACTIVATE_REBOOT_DELAY_SECOND);
				 ...
				 uint8_t *resp, uint16_t *resp_len, void *ext_params)
{
	...
	*resp_len = sizeof(struct pldm_activate_firmware_resp);
	...
	/*
	 * Activation is medium-specific reset (see design.md): nothing here
	 * reboots the board. Give the platform ACTIVATE_REBOOT_DELAY_SECOND
	 * to reset on its own after this response is sent; if it hasn't,
	 * force a chip reset so the newly written image actually takes
	 * effect.
	 */
	k_thread_create(&activate_reboot_thread, activate_reboot_stack,
			K_THREAD_STACK_SIZEOF(activate_reboot_stack), activate_reboot_handler,
			NULL, NULL, NULL, 3, 0, K_SECONDS(ACTIVATE_REBOOT_DELAY_SECOND));
			  ...
			  uint8_t *resp, uint16_t *resp_len, void *ext_params)
{
	...
	*resp_len = sizeof(struct pldm_get_status_resp);
	...
	LOG_HEXDUMP_INF(resp_p, sizeof(*resp_p), "Get status");
exit:
	return PLDM_SUCCESS;
}
