...
LOG_MODULE_REGISTER(pldm, LOG_LEVEL_INF);
...
uint8_t mctp_pldm_cmd_handler(void *mctp_p, uint8_t *buf, uint32_t len, mctp_ext_params ext_params)
{
	...
	LOG_DBG("PLDM %s: type=0x%x cmd=0x%x inst=0x%x", hdr->rq ? "req" : "resp",
		hdr->pldm_type, hdr->cmd, hdr->req_d_id & 0x1F);
...
*/
	...
	uint8_t *resp_buf = malloc(PLDM_MAX_DATA_SIZE);
	if (!resp_buf) {
		LOG_ERR("Failed to allocate response buffer!");
		return PLDM_ERROR;
	}
	memset(resp_buf, 0, PLDM_MAX_DATA_SIZE);
	/*
...
send_msg:
	...
	LOG_DBG("PLDM resp: queuing type=0x%x cmd=0x%x len=%u", hdr->pldm_type, hdr->cmd,
		resp_len);
	rc = mctp_send_msg(mctp_inst, resp_buf, resp_len, ext_params);
	free(resp_buf);
	return rc;
}
...
uint8_t mctp_pldm_send_msg(void *mctp_p, pldm_msg *msg)
{
	...
	*/
	...
	if (msg->hdr.rq) {
		...
		if (!p) {
			LOG_WRN("wait_msg alloc failed size=%d!", sizeof(*p));
			goto error;
		}
...
K_THREAD_DEFINE(pldm_wait_resp_to, 1024, pldm_msg_timeout_monitor, NULL, NULL, NULL, 2, 0, 0);
