...
LOG_MODULE_REGISTER(monitor_venice, CONFIG_LOG_DEFAULT_LEVEL);
 ...
 */
static void platform_venice_reset_handler(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
...
static void platform_venice_reset_monitor_init(void)
{
	int ret;
	struct gpio_dt_spec rst_pltrst =
		GPIO_DT_SPEC_GET_BY_IDX(DT_INST(0, aspeed_pfr_gpio_venice), rst_pltrst_in_gpios, 0);
	...
	gpio_init_callback(&rst_pltrst_cb_data, platform_venice_reset_handler, BIT(rst_pltrst.pin));
	ret = gpio_add_callback(rst_pltrst.port, &rst_pltrst_cb_data);
	LOG_INF("Platform: gpio_add_callback = %d", ret);
}

static void platform_venice_reset_monitor_remove(void)
{
	struct gpio_dt_spec rst_pltrst =
		GPIO_DT_SPEC_GET_BY_IDX(DT_INST(0, aspeed_pfr_gpio_venice), rst_pltrst_in_gpios, 0);
...
void platform_venice_monitor_init(void)
{
	platform_venice_reset_monitor_init();
}

void platform_venice_monitor_remove(void)
{
	platform_venice_reset_monitor_remove();
}

static const struct platform_monitor_ctrl_ops venice_platform_monitor_ops = {
	.init = platform_venice_monitor_init,
	.remove = platform_venice_monitor_remove,
};

const struct platform_monitor_ctrl_ops *get_platform_monitor_ctrl_ops(void)
{
	return &venice_platform_monitor_ops;
}
