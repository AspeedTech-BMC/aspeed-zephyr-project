...
#include "platform/platform_monitor_ctrl.h"
...
{
	const struct platform_monitor_ctrl_ops *ops = get_platform_monitor_ctrl_ops();

	if (ops->init) {
		ops->init();
	} else {
		LOG_WRN("Platform monitor init callback is not registered");
	}
}
...
void platform_monitor_remove(void)
{
	...
	if (ops->remove) {
		...
		LOG_WRN("Platform monitor remove callback is not registered");
	}
}
...
void power_btn(bool enable)
{
	...
	if (ops->power_btn) {
		ops->power_btn(enable);
	} else {
		LOG_WRN("Power button callback is not registered");
	}
}
