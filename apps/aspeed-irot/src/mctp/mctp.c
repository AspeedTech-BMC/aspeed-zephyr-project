...
LOG_MODULE_REGISTER(mctp, LOG_LEVEL_INF);
...
static uint8_t set_thread_name(mctp *mctp_inst)
{
	...
	case MCTP_MEDIUM_TYPE_TARGET_I3C:
		...
		break;
#if defined(CONFIG_IPM)
	case MCTP_MEDIUM_TYPE_IPC:
		LOG_DBG("medium_type: ipc");
		snprintf(mctp_inst->mctp_rx_task_name, sizeof(mctp_inst->mctp_rx_task_name),
			 "mctprx_%02x", mctp_inst->medium_type);
		snprintf(mctp_inst->mctp_tx_task_name, sizeof(mctp_inst->mctp_tx_task_name),
			 "mctptx_%02x", mctp_inst->medium_type);
		break;
#endif
	default:
		return MCTP_ERROR;
...
#if defined(CONFIG_IPM)
	case MCTP_MEDIUM_TYPE_IPC:
		ret = mctp_ipc_init(mctp_inst, medium_conf);
		break;
#endif
	default:
		return MCTP_ERROR;
...
static uint8_t mctp_pkt_assembling(mctp *mctp_inst, uint8_t *buf, uint16_t len)
{
	...
	if (hdr->som && !hdr->eom) {
		...
		*buf_p = (uint8_t *)malloc(mctp_inst->max_msg_size * 4);
		...
		memset(*buf_p, 0, mctp_inst->max_msg_size * 4);
	}
...
/* one allocation covers both the list linkage and the receive payload */
struct mctp_rx_buf {
	sys_snode_t node;
	uint8_t data[];
};
...
static void mctp_rx_task(void *arg, void *dummy0, void *dummy1)
{
	...
	while (1) {
		struct mctp_rx_buf *rx_node =
			malloc(sizeof(struct mctp_rx_buf) + mctp_inst->max_msg_size);
		if (!rx_node) {
			LOG_ERR("mctp_rx_task: failed to allocate rx buffer");
			k_msleep(10);
			continue;
		}
		sys_slist_append(&mctp_inst->rx_buf_list, &rx_node->node);

		uint8_t *read_buf = rx_node->data;
		...
		if (!read_len) {
			sys_slist_find_and_remove(&mctp_inst->rx_buf_list, &rx_node->node);
			free(rx_node);
			continue;
		}
		...
		/* payload copied into temp_msg_buf; rx_node no longer needed for mid-packet */
		if (!hdr->eom) {
			sys_slist_find_and_remove(&mctp_inst->rx_buf_list, &rx_node->node);
			free(rx_node);
			continue;
...
static void mctp_tx_task(void *arg, void *dummy0, void *dummy1)
{
	...
	while (1) {
		...
		LOG_DBG("mctp tx task get msgq ret = %d", ret);
		if (ret)
			continue;

		if (!mctp_msg.buf) {
			LOG_ERR("mctp tx task receive empty buffer");
			mctp_tx_task_response(mctp_msg.evt_msgq, MCTP_ERROR);
			continue;
		}

		if (!mctp_msg.len) {
			LOG_ERR("mctp tx task receive empty length");
			free(mctp_msg.buf);
			mctp_tx_task_response(mctp_msg.evt_msgq, MCTP_ERROR);
			continue;
		}
		...
		/*
		 * The bridge message already has the mctp transport header, and the bridge
		...
		for (i = 0; i < split_pkt_num; i++) {
			 ...
			 * response packets? If the message is response, keep the
			 * original msg_tag of ext_params.
			 */
...
mctp *mctp_init(void)
{
	...
	if (!mctp_inst) {
		LOG_ERR("mctp init failed, malloc mctp_inst failed");
		return NULL;
	}
...
uint8_t mctp_stop(mctp *mctp_inst)
{
	...
	/* free any rx buffer that was mid-flight when the thread was aborted */
	sys_snode_t *node;
	while ((node = sys_slist_get(&mctp_inst->rx_buf_list)) != NULL)
		free(CONTAINER_OF(node, struct mctp_rx_buf, node));
				 ...
				 mctp_ext_params ext_params, uint8_t is_bridge)
{
	...
	if (!mctp_msg.buf) {
		LOG_WRN("mctp tx task malloc buf failed");
		goto error;
	}
