...
LOG_MODULE_REGISTER(cptra_test, LOG_LEVEL_DBG);

#define CONFIG_CPTRA_SAMPLE_SSP 1
...
#define IDEVID_CSR_BASE				0x74bbf000
...
__attribute__((unused)) static void cptra_test_set_auth_manifest(void)
{
	...
	LOG_INF("manifest_size:0x%x", input.manifest_size);
	LOG_INF("manifest_marker:0x%x", input.preamble.manifest_marker);
	LOG_INF("preamble_size:0x%x", input.preamble.preamble_size);
	LOG_INF("metadata_entry_entry_count:0x%x", input.metadata_entry_entry_count);
	...
	LOG_INF("Hashing manifest_version and manifest_flags");
	...
	if (ret) {
		...
		LOG_INF("Raw compare - PASS");

	LOG_INF("Populate manifest_vendor_ecc384_sig");
	...
	LOG_INF("manifest_vendor_ecc384_sig addr: 0x%x 0x%x",
		(uint32_t)input.preamble.manifest_vendor_ecc384_sig,
		(uint32_t)(input.preamble.manifest_vendor_ecc384_sig + 12));
	LOG_HEXDUMP_INF(input.preamble.manifest_vendor_ecc384_sig, 48, "r:");
	LOG_HEXDUMP_INF(input.preamble.manifest_vendor_ecc384_sig + 12, 48, "s:");

	LOG_INF("Verify & Populate vendor signature successfully");

	LOG_INF("Hashing owner pub keys");
	memset(hash, 0, sizeof(hash));
	...
	LOG_INF("Populate manifest_owner_ecc384_sig");
	...
	LOG_INF("manifest_owner_ecc384_sig addr: 0x%x 0x%x",
		(uint32_t)input.preamble.manifest_owner_ecc384_sig,
		(uint32_t)(input.preamble.manifest_owner_ecc384_sig + 12));
	LOG_HEXDUMP_INF(input.preamble.manifest_owner_ecc384_sig, 48, "r:");
	LOG_HEXDUMP_INF(input.preamble.manifest_owner_ecc384_sig + 12, 48, "s:");

	LOG_INF("Verify & Populate owner signature successfully");

	LOG_INF("Hashing metadata entries");
	memset(hash, 0, sizeof(hash));
	...
	LOG_INF("Populate metadata_owner_ecc384_sig");
	...
	LOG_INF("metadata_owner_ecc384_sig addr: 0x%x 0x%x",
		(uint32_t)input.preamble.metadata_owner_ecc384_sig,
		(uint32_t)(input.preamble.metadata_owner_ecc384_sig + 12));
	LOG_HEXDUMP_INF(input.preamble.metadata_owner_ecc384_sig, 48, "r:");
	LOG_HEXDUMP_INF(input.preamble.metadata_owner_ecc384_sig + 12, 48, "s:");
	LOG_INF("Verify & Populate metadata signature successfully");
	...
	if (ret) {
		...
		LOG_INF("caliptra_set_auth_manifest is successful");

	LOG_INF("output: chksum:0x%x, fips_status:0x%x",
		output.chksum, output.fips_status);
...
// Split each test into individule shell command for SSP sample

static int cmd_cptra_sha384(const struct shell *shell, size_t argc, char **argv)
{
	cptra_test_sha384();
	return 0;
}

static int cmd_cptra_ecdsa(const struct shell *shell, size_t argc, char **argv)
{
	cptra_test_ecdsa_verify();
	return 0;
}

static int cmd_cptra_lms(const struct shell *shell, size_t argc, char **argv)
{
	...
	/* cptra: test dice */
static int cmd_stash_measurement(const struct shell *shell, size_t argc, char **argv)
{
	cptra_test_stash_measurement();
	return 0;
}

static int cmd_quote_pcrs(const struct shell *shell, size_t argc, char **argv)
{
	cptra_test_quote_pcrs();
	return 0;
}

static int cmd_extend_pcr(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_extend_pcr();
	return 0;
}

static int cmd_incr_pcr(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_increment_pcr_reset_counter();
	return 0;
}

static int cmd_dpe_tag_tci(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_dpe_tag_tci();
	return 0;
}

static int cmd_dpe_get_tagged_tci(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_dpe_get_tagged_tci();
	return 0;
}

static int cmd_add_subject_alt_name(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_add_subject_alt_name();
	return 0;
}

static int cmd_certify_key_extended(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_certify_key_extended();
	return 0;
}

static int cmd_invoke_dpe_cmd(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_invoke_dpe_command();
	return 0;
}

static int cmd_disable_attestation(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_disable_attestation();
	return 0;
}

static int cmd_get_idev_cert(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_get_idev_cert();
	return 0;
}

static int cmd_populate_idev_cert(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_populate_idev_cert();
	return 0;
}

static int cmd_get_idev_info(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_get_idev_info();
	return 0;
}

static int cmd_get_ldev_cert(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_get_ldev_cert();
	return 0;
}

static int cmd_get_fmc_alias_cert(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_get_fmc_alias_cert();
	return 0;
}

static int cmd_get_rt_alias_cert(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_get_rt_alias_cert();
	return 0;
}
	...
	/* cptra: test misc */
static int cmd_fw_info(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_fw_info();
	return 0;
}	
static int cmd_capabilities(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_capabilities();
	return 0;
}
...
static int cmd_set_auth_manifest(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_set_auth_manifest();
	return 0;
}
static int cmd_authorize_and_stash(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_authorize_and_stash();
	return 0;
}
static int cmd_get_idevid_csr(const struct shell *shell, size_t argc, char **argv) {
	cptra_test_get_idevid_csr();
	return 0;
}
static int cmd_sign_with_exported_ecdsa(const struct shell *shell, size_t argc, char **argv) {
	uint8_t exported_cdi[32] = {0};
	cptra_test_sign_with_exported_ecdsa(exported_cdi);
	return 0;
}
static int cmd_revoke_exported_cdi_handle(const struct shell *shell, size_t argc, char **argv) {
	uint8_t exported_cdi[32] = {0};
	cptra_test_revoke_exported_cdi_handle(exported_cdi);
	return 0;
}
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_cptra,
	SHELL_CMD(test, NULL, "Run all Caliptra tests", cmd_cptra),
	SHELL_CMD(sha384, NULL, "Run SHA384 test", cmd_cptra_sha384),
	SHELL_CMD(ecdsa, NULL, "Run ECDSA verify test", cmd_cptra_ecdsa),
	SHELL_CMD(lms, NULL, "Run LMS verify test", cmd_cptra_lms),
	SHELL_CMD(stash_measurement, NULL, "Run stash measurement test", cmd_stash_measurement),
	SHELL_CMD(quote_pcrs, NULL, "Run quote PCRs test", cmd_quote_pcrs),
	SHELL_CMD(extend_pcr, NULL, "Run extend PCR test", cmd_extend_pcr),
	SHELL_CMD(incr_pcr, NULL, "Run increment PCR reset counter test", cmd_incr_pcr),
	SHELL_CMD(dpe_tag_tci, NULL, "Run DPE tag TCI test", cmd_dpe_tag_tci),
	SHELL_CMD(dpe_get_tagged_tci, NULL, "Run DPE get tagged TCI test", cmd_dpe_get_tagged_tci),
	SHELL_CMD(add_subject_alt_name, NULL, "Run add subject alt name test", cmd_add_subject_alt_name),
	SHELL_CMD(certify_key_extended, NULL, "Run certify key extended test", cmd_certify_key_extended),
	SHELL_CMD(invoke_dpe_cmd, NULL, "Run invoke DPE command test", cmd_invoke_dpe_cmd),
	SHELL_CMD(disable_attestation, NULL, "Run disable attestation test", cmd_disable_attestation),
	SHELL_CMD(get_idev_cert, NULL, "Run get iDev cert test", cmd_get_idev_cert),
	SHELL_CMD(populate_idev_cert, NULL, "Run populate iDev cert test", cmd_populate_idev_cert),
	SHELL_CMD(get_idev_info, NULL, "Run get iDev info test", cmd_get_idev_info),
	SHELL_CMD(get_ldev_cert, NULL, "Run get lDev cert test", cmd_get_ldev_cert),
	SHELL_CMD(get_fmc_alias_cert, NULL, "Run get FMC alias cert test", cmd_get_fmc_alias_cert),
	SHELL_CMD(get_rt_alias_cert, NULL, "Run get RT alias cert test", cmd_get_rt_alias_cert),
	SHELL_CMD(fw_info, NULL, "Run FW info test", cmd_fw_info),
	SHELL_CMD(capabilities, NULL, "Run capabilities test", cmd_capabilities),
	SHELL_CMD(version, NULL, "Run version test", cmd_version),
	SHELL_CMD(set_auth_manifest, NULL, "Run set auth manifest test", cmd_set_auth_manifest),
	SHELL_CMD(authorize_and_stash, NULL, "Run authorize and stash test", cmd_authorize_and_stash),
	SHELL_CMD(get_idevid_csr, NULL, "Run get iDevID CSR test", cmd_get_idevid_csr),
	SHELL_CMD(sign_with_exported_ecdsa, NULL, "Run sign with exported ECDSA test", cmd_sign_with_exported_ecdsa),
	SHELL_CMD(revoke_exported_cdi_handle, NULL, "Run revoke exported CDI handle test", cmd_revoke_exported_cdi_handle),


	SHELL_SUBCMD_SET_END /* Array terminated. */
);
SHELL_CMD_REGISTER(cptra, &sub_cptra, "Caliptra demo command", NULL);
#endif
