                        ...
                        uint8_t *signature, size_t *sig_size)
{
	...
	memcpy(sign_input->digest, message_hash, hash_size);

	LOG_HEXDUMP_INF(sign_input->digest, digest_len, "digest to sign:");
	...
	LOG_HEXDUMP_INF(sign_output->signature_r, sign_len / 2, "signature_r:");
	LOG_HEXDUMP_INF(sign_output->signature_s, sign_len / 2, "signature_s:");

	memcpy(signature, sign_output->signature_r, sign_len / 2);
	memcpy(signature + sign_len / 2, sign_output->signature_s, sign_len / 2);

	LOG_HEXDUMP_INF(signature, sign_len, "signature (r||s):");
