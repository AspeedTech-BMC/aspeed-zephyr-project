...
#define MIN_FW_UPDATE_BASELINE_TRANS_SIZE (4096)
	...
	*resp_len = 1;
	...
	switch (req_p->comp_classification) {
		...
		case PLDM_COMP_FIRMWARE:
			if (req_p->comp_identifier < 0x2700 || req_p->comp_identifier > 0x27FF) {
				LOG_ERR("Invalid component identifier id for firmware component");
				resp_p->completion_code = PLDM_ERROR_INVALID_DATA;
				goto exit;
			}
			break;
