...
#define NAK 0x15
#define CAN 0x18
#define MEOF 0x1A		/* ^Z for DOS officionados */
...
#define xyzModem_CHAR_TIMEOUT            3000	/* 3 seconds */
...
static int CYGACC_COMM_IF_GETC_TIMEOUT(char chan, char *c)
{
	...
	while (uart_poll_in(g_uart_dev, c) == -1) {
		if (k_uptime_get() - now_ms > xyzModem_CHAR_TIMEOUT)
			return 0;
	}
...
static int xyzModem_get_hdr (void)
{
	...
	/* Verify checksum/CRC */
	if (xyz.crc_mode) {
		cksum = crc16_ccitt_modem(0, xyz.pkt, xyz.len);
		if (cksum != ((xyz.crc1 << 8) | xyz.crc2)) {
			ZM_DEBUG(zm_dprintf("CRC error - recvd: %02x%02x, computed: %x\n",
						xyz.crc1, xyz.crc2, cksum & 0xFFFF));
			CYGACC_COMM_IF_PUTC('7', '8');
...
int xyzModem_stream_open(struct device *dev, connection_info_t *info, int *err)
{
	int stat = 0;
				...
				if (stat == 0) {
					if (xyz.blk == xyz.next_blk) {
						...
						if (xyz.mode == xyzModem_xmodem || xyz.file_length == 0) {
							...
							if ((xyz.bufp[xyz.len - 1] == MEOF) &&
								(xyz.bufp[xyz.len - 2] == MEOF) &&
								(xyz.bufp[xyz.len - 3] == MEOF)) {
								while (xyz.len && (xyz.bufp[xyz.len - 1] == MEOF))
									xyz.len--;
							}
						}
