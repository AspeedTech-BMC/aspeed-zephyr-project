...
int wdt_init(struct ast_chip *chip)
{
	...
	if (chip->rev_id) {
		...
		for (idx = 0; idx < 8; idx++) {
			...
			wdt_writel(0x003e0000, wdt_base_addr + WDT_RST_MASK_5);

			/* SW reset mask */
			wdt_writel(0x8207e771, wdt_base_addr + WDT_SW_RST_MASK_1);
			...
			wdt_writel(0x003e0000, wdt_base_addr + WDT_SW_RST_MASK_5);
		}
	}
